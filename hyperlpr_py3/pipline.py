...
def drawRectBox(image,rect,addText):
    ...
    cv2.rectangle(image, (int(rect[0]-1), int(rect[1])-30), (int(rect[0]+130), int(rect[1])), (0, 0, 255), 2, cv2.LINE_AA)

    img = Image.fromarray(image)
    draw = ImageDraw.Draw(img)
    # draw.text((int(rect[0]+1), int(rect[1]-16)), addText, (255, 255, 255), font=fontC)
    draw.text((int(rect[0]+1), int(rect[1]-30)), addText, (255, 255, 0), font=fontC)
    imagex = np.array(img)
